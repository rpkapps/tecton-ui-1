...
<AppShellHeader>
  ...
  <AppShellActions>
    ...
    <AppShellAction label="Help"><CircleHelpIcon /></AppShellAction>
...
`AppShellAction` is a ghost icon button whose `label` is both the accessible name and the tooltip; `shortcut` adds a key hint to the tooltip. A hint is only a label: Tecton binds no keys, so show one only for a key the application handles itself. `AppShellCommandTrigger` looks like a search field from `md` up and collapses to an icon button below it. `AppShellOverflow` is the "More" menu for the actions a narrow header cannot show, and `AppShellDivider` is the hairline between groups.
...
| `shortcut` | `ReactNode` | - | Key hint rendered as a `Kbd` in the tooltip (`"?"`). A label only: it binds no key. |
...
| `shortcut` | `ReactNode` | - | Key hint rendered as a `Kbd` at the end (`"⌘K"`). A label only: it binds no key. |
