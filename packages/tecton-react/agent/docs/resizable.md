...
- `ResizablePanelGroup` wraps react-resizable-panels: the axis prop is `orientation`, `defaultSize` / `minSize` / `maxSize` take unit strings (`"25%"`, `"320px"`), and disabling is `disabled`.
