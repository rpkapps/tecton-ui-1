...
Set `showValue` to render the formatted value in the centre, or pass children for custom content. `min`, `max` and `format` (`Intl.NumberFormatOptions`) set the scale and the text.

<ComponentPreview name="circular-progress-value" />

## Indeterminate

Pass `value={null}` for a spinning ring while the amount of work is unknown. With reduced motion requested the ring keeps turning slowly rather than stopping: a still arc would read as a stuck value.
...
| --- | --- | --- | --- |
| `value` | `number \| null` | - | Current value; `null` is indeterminate (the ring spins). |
