...
import { compareKeys } from "./src/compare/keys.ts"
...
const config = defineConfig(({ command }) => ({
  ...
  plugins: [
    // Must run before tanstackStart/react so .mdx files are transformed first.
    // Generates .source/server.ts and .source/browser.ts from the collections
    // in source.config.ts (also generated by `fumadocs-mdx` on postinstall,
    // for tsc and eslint).
    fumadocsMdx({ macro: false }),
    tailwindcss(),
    tanstackStart({
      // Pages no crawled link reaches: every block detail page (the gallery
      // links a block to its own anchor) and the chrome-less compare matrices.
      pages: [
        ...blockNames.map((name) => ({ path: `/blocks/${name}` })),
        ...compareKeys.map((key) => ({ path: `/compare/${key}` })),
      ],
      prerender: {
        enabled: true,
        crawlLinks: true,
        ...
        // contention.
        concurrency: 4,
        // A page that fails to render (a broken link the crawler follows, a
        // loader that throws) must fail the build. No retries: the plugin
        // re-queues a failed page through the same de-duplicating crawl
        // queue, so a retry never runs and the error is silently dropped.
        retryCount: 0,
        failOnError: true,
