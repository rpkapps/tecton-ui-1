...
`Canvas` is `flex-1` and fills the remaining height of a flex column such as `AppShellMain`; give it a height when it is embedded. Overlays let pointer events through between their children, so the surface stays interactive around the floating controls.
...
| --- | --- | --- | --- |
| `position` | `"top-left" \| "top" \| "top-right" \| "left" \| "right" \| "bottom-left" \| "bottom" \| "bottom-right"` | `"top-left"` | Edge or corner the overlay is pinned to. `top` and `bottom` stack children horizontally, the others vertically. |
