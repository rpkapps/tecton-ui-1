---
title: TanStack Form
description: Build forms in React using TanStack Form, Zod and the Tecton form controls.
links:
...
Every example uses the Tecton controls as they are. If you are porting a form from another component library, note the prop names: Tecton's controlled inputs use `isSelected` / `onChange`, `selectedKey` / `onSelectionChange` and `isInvalid` where others use `checked` / `onCheckedChange`, `value` / `onValueChange` and `aria-invalid`. [Working with different field types](#working-with-different-field-types) lists them per control.
...
- Add `aria-invalid` to the controls that render a native element (`<Input />`, `<Textarea />`), and `isInvalid` to the composed ones (`<Select />`, `<Checkbox />`, `<RadioGroup />`).
...
- For select components, use `selectedKey` and `onSelectionChange` on the `<Select />` component. `Select` uses `null` for "nothing selected", so map the empty string to `null` and back.
...
- To show errors, add the `data-invalid` prop to the `<Field />` component (`Switch` has no invalid state of its own).
