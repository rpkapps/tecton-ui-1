...
# Accordion — @tecton/react/components/accordion
...
- one region that opens and closes on its own → `Collapsible` (tecton docs collapsible)
- sections that replace each other in the same space → `Tabs` (tecton docs tabs)
- an expandable hierarchy of folders and records → `TreeView` (tecton docs tree-view)
