...
- **Icons:** the 18 domain glyphs are generated from the Tecton icon export in `packages/tecton-react/icons-src/tecton/`; the export's general-purpose glyphs are not generated, because lucide-react already covers them. Replace those files with a newer export and run `icons:build`.
