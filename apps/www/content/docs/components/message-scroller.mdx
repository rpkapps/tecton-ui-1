...
upstream: apps/v4/content/docs/components/base/message-scroller.mdx
---
...
- a styled scroll region for content that does not grow at its end: use [ScrollArea](/docs/components/scroll-area)
- the row layout of one turn inside the transcript: use [Message](/docs/components/message)
...
<Dont severity="CRITICAL" title="A transcript in a plain overflow-auto div">
...
<Dont severity="CRITICAL" title="The scroller frame without its provider">
...
<Dont severity="HIGH" title="Rows dropped straight into the content">
