...
`DrawerContent` composes the portal, overlay, viewport, and popup. For lower-level control, `DrawerPortal`, `DrawerOverlay`, and `DrawerSwipeHandle` are also exported.
...
<Dont severity="HIGH" title="isOpen instead of open on the drawer">
