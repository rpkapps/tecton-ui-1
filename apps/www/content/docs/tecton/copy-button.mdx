...
The shadcn `Button` with clipboard behaviour; accepts all `Button` props except `onClick`. While the copied or failed state shows, it carries `data-copied` or `data-error`.
