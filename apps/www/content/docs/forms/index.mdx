---
title: Forms
description: Build forms with React Aria components and a headless form library.
---
...
<div className="mt-8 grid gap-4 sm:grid-cols-2 sm:gap-6">
  ...
  <LinkedCard href="https://ui.shadcn.com/docs/forms/react-hook-form" target="_blank" rel="noreferrer" className="border border-dashed bg-transparent">
    ...
    <p className="mt-1 text-xs text-muted-foreground">Upstream guide (Radix examples)</p>
  </LinkedCard>
</div>
...
The upstream React Hook Form guide applies as well; only the control props differ (see the [TanStack Form guide](/docs/forms/tanstack-form#working-with-different-field-types) for the React Aria equivalents of `onValueChange` / `onCheckedChange`).
