...
- a one-line search or filter box → `InputGroup` (tecton docs input-group)
- a form field for a long description → `Textarea` (tecton docs textarea)
- a structured question with choices for the user to answer → `Questionnaire` (tecton docs questionnaire)
...
- The message box is a `Composer` (`ComposerField` holding `ComposerInput` and a `ComposerToolbar` ending in `ComposerSubmit`, with `ComposerHint` and `ComposerStatusMessage`), fed the chat's `status` and `onStop`, not an `InputGroupInput` with a hand-written Enter handler.

Related: message-scroller, message, attachment, questionnaire
