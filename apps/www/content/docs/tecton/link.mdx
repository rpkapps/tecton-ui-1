...
Set `isExternal` to open in a new tab with `rel="noreferrer noopener"` and append an icon. A `rel` you pass (`nofollow`) is added to `noreferrer noopener`, never swapped for it, and an explicit `target` wins over the default `_blank`.
...
| `isExternal` | `boolean` | false | Opens in a new tab (`target="_blank"` unless `target` is set), adds `noreferrer noopener` to `rel` and shows an icon. |
