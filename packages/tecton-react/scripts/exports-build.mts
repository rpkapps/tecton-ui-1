...
/**
 * Modules the build emits but the package does not expose. `components/direction`
 * is generated by the shadcn CLI and re-exports the underlying primitive's
 * direction provider; applications set the direction with `TectonProvider`
 * (`tecton/provider`), which every Tecton component reads. `src/tecton/internal/`
 * is never listed: only top-level files of a directory become entries.
 */
const MODULE_INTERNALS = new Set(["components/direction"]);
