...
It follows the [upstream shadcn/ui guide](https://ui.shadcn.com/docs/forms/tanstack-form); the examples are rewritten for the React Aria components, whose controls use `isSelected` / `onChange` and `selectedKey` / `onSelectionChange` instead of the Radix `checked` / `onCheckedChange` and `value` / `onValueChange`. Invalidity is not uniform: `<Input />` and `<Textarea />` wrap real DOM elements and take `aria-invalid`, the React Aria controls take `isInvalid`, and `<Switch />` takes neither — see [Displaying errors](#displaying-errors).
...
- Add `aria-invalid` to native controls such as `<Input />` and `<Textarea />`, and `isInvalid` to the React Aria controls (`<Select />`, `<Checkbox />`, `<RadioGroup />`, `<Combobox />`).
- `<Switch />` is the exception: React Aria's `SwitchProps` omits `isInvalid` and `isRequired`, so mark it with `data-invalid` (as the [Switch](/docs/components/switch) invalid example does).
