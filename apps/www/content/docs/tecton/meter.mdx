...
<Callout>
  The segmented gauge from the FDA and Well Design cards is a Tecton pattern of its own; [Progress](/docs/components/progress) is kept for task progress.
</Callout>
...
An accessible meter rendered as segments. Accepts `value`, `minValue`, `maxValue` and `formatOptions`.
