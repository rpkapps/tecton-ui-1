...
A `role="toolbar"` on a floating surface. It is one tab stop: the arrow keys move between its `CanvasToolbarButton`s (Up / Down when vertical, Left / Right when horizontal, mirrored in right-to-left layouts), stop at the ends, and Tab leaves the rail. Give it an `aria-label`.

| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `orientation` | `"vertical" \| "horizontal"` | "vertical" | Direction of the rail, and of the arrow keys that move along it. |
| `disabled` | `boolean` | false | Disables every tool in the rail. |
| `aria-label` | `string` | - | Accessible name of the toolbar. |

### CanvasToolbarButton

A tool in the rail: a `Button` (`variant="ghost"`, `size="icon-sm"` by default) that takes part in the rail's arrow-key navigation. Accepts `Button` props; a disabled tool stays focusable so the arrows do not skip it (`focusableWhenDisabled={false}` removes it). Compose it with a trigger through `render`:
