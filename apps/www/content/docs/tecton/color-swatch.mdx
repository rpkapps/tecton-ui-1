---
title: Color Swatch
description: "A colour preview with optional label and detail text, for colour tags, legends and theme documentation; editable with a preset picker."
---

<ComponentPreview name="color-swatch-demo" />

## Usage

```tsx
import { ColorSwatch } from "@tecton/react/tecton/color-swatch"
```

```tsx
<ColorSwatch color="#f59e0b" label="Sandstone" detail="#f59e0b" />
```

The swatch is announced as an image named by your `aria-label` (or a string `label`) followed by a generated English colour name, such as "Sandstone, vibrant orange"; `colorName` replaces the generated name.

## Sizes

<ComponentPreview name="color-swatch-sizes" />

## Shapes

<ComponentPreview name="color-swatch-shapes" />

## Labels

`label` and `detail` render text next to the swatch; the detail is set in the mono font.

<ComponentPreview name="color-swatch-labels" />

## Editable

`onColorChange` turns the swatch into a button that opens a picker: preset colours (the Tecton accents, `colorSwatchPresets`, unless you pass `presets`), the platform colour picker and a hex field. It receives the new colour as `#RRGGBB`. Arrow keys in the presets select as they move, so each press calls `onColorChange`; the hex field commits on Enter or blur and accepts any CSS colour.

<ComponentPreview name="color-swatch-editable" />

{/* guidelines:start */}

## Usage guidelines

### Use it when

- The colour *is* the data: a horizon colour, a facies legend, a theme token, a series fill.
- The colour needs its name or its value beside it: `label` and `detail`.
- The user has to change it: `onColorChange` turns the swatch into a button with a preset and hex picker.

### Not for

- a label that names a status or a category in words: use [Badge](/docs/components/badge)
- the colour key for the series of a chart: use [ChartLegendContent](/docs/components/chart)
- a count or a dot pinned to the corner of an icon or an avatar: use [CountBadge](/docs/tecton/count-badge)

### Do

<DoList>

<Do>

Pass any CSS colour to `color` — a hex, `rgb()`, `hsl()`, `oklch()`, a keyword or `var(--tecton-color-accent-lime-fill)`; a translucent colour shows over a checkerboard.

</Do>

<Do>

Size with `size="xs" | "sm" | "md" | "lg" | "xl"` (12 to 48 px) and pick the corner with `shape="square" | "rounded" | "circle"`.

</Do>

<Do>

Name what the colour stands for with `aria-label` (or a string `label`); the swatch appends the colour's English name itself ("Sandstone, vibrant orange"), and `colorName` replaces that name.

</Do>

<Do>

Make it editable with `onColorChange`, which receives `#RRGGBB`, and replace the Tecton accent presets with `presets` when the palette is domain-specific; the presets are a radio group, so an arrow key selects as it moves.

</Do>

</DoList>

### Don't

<Dont severity="HIGH" title="A coloured div in place of the swatch">

<Wrong>

```tsx
<div
  className="size-6 rounded-md"
  style={{ background: horizon.color }}
/>
```

</Wrong>

<Correct>

```tsx
<ColorSwatch
  color={horizon.color}
  size="md"
  aria-label={`${horizon.name} colour`}
/>
```

</Correct>

The swatch is `role="img"` with an accessible name and carries `border border-border-subtle shadow-xs`, which is what keeps a white, pale or transparent colour visible against the surface; a bare div is silent to assistive technology and its inline `style` is what `no-inline-styles` reports.

</Dont>

<Dont severity="HIGH" title="Opening the picker with a click handler">

<Wrong>

```tsx
<ColorSwatch color={color} onClick={() => setPickerOpen(true)} />
```

</Wrong>

<Correct>

```tsx
<ColorSwatch color={color} onColorChange={setColor} aria-label="Series colour" />
```

</Correct>

Without `onColorChange` the swatch is a non-focusable `role="img"` `span`, so an `onClick` on it is out of reach of the keyboard and of assistive technology; `onColorChange` wraps it in the `color-swatch-trigger` button that opens the preset radio group and the hex field.

</Dont>

<Dont severity="MEDIUM" title="Label and detail written as sibling spans">

<Wrong>

```tsx
<div className="flex items-center gap-2">
  <ColorSwatch color="#f59e0b" />
  <span className="text-sm">Sandstone</span>
  <span className="font-mono text-xs text-zinc-500">#f59e0b</span>
</div>
```

</Wrong>

<Correct>

```tsx
<ColorSwatch color="#f59e0b" label="Sandstone" detail="#f59e0b" />
```

</Correct>

With `label` and `detail` the component renders the `color-swatch-item` row itself — name over detail, the detail in the mono face and `muted-foreground`, `className` moved from the swatch to the row — while `text-zinc-500` is stock Tailwind that emits no CSS at all.

</Dont>

{/* guidelines:end */}

## API Reference

### ColorSwatch

Other props go to the swatch `span`.

| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `color` | `string` | `"transparent"` | Any CSS colour, including `var(--token)`. |
| `colorName` | `string` | generated | Replaces the colour name in the accessible name. |
| `size` | `"xs" \| "sm" \| "md" \| "lg" \| "xl"` | `"md"` | 12 / 16 / 24 / 32 / 48 px. |
| `shape` | `"square" \| "rounded" \| "circle"` | `"rounded"` | Corner radius. |
| `label` | `ReactNode` | - | Text next to the swatch. |
| `detail` | `ReactNode` | - | Secondary mono text, e.g. the hex value. |
| `onColorChange` | `(color: string) => void` | - | Makes the swatch editable; called with the new colour as `#RRGGBB`. |
| `presets` | `string[]` | `colorSwatchPresets` | Preset colours of the picker; values that resolve to no colour are left out. |
| `aria-label` | `string` | - | What the colour stands for; names the button when editable (default "Edit colour"). |
