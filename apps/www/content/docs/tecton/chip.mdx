...
<Callout title="Chip or Badge?">
  A static label is a [Badge](/docs/components/badge), which carries the Tecton status colours, the outline appearance and the size scale. `Chip` is the *interactive* label: a tag inside a `ChipGroup` that can be selected and removed with the keyboard. It is styled with the same `badgeVariants`, so a chip and a badge with the same props look identical. What Tecton calls a *Badge* (a count anchored to an icon) is the [CountBadge](/docs/tecton/count-badge) component.
</Callout>
...
The group wrapper: owns selection, removal and the keyboard behaviour of the whole row, and applies Tecton spacing.
...
The list of chips. Pass `items` and a render function, or static `Chip` children.
...
A single chip, styled with the Badge variants. Give it an `id` so the group can address it in `selectedKeys` and `onRemove`.
...
A small button with an `X` icon. It is rendered automatically inside a `Chip` when the group has `onRemove`; pass `children` to replace the icon.
