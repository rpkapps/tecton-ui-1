...
| --- | --- | --- | --- |
| `SeismicBackground` | Seismogram record | Scrolls slowly to the left | Hero and summary cards |
...
| `FlowBackground` | Process-flow schematic: pipe runs, elbows, junctions | Fluid pulses move through the pipes | Pipelines and process screens; the liveliest one |
| `WellLogBackground` | Vertical log traces | Pages upwards very slowly, as if scrolling deeper | Sidebars, login panels |
...
`intensity` caps the effect's peak opacity (`low` 12%, `medium` 20%, `high` 32%) so text over it keeps its contrast; `speed` sets the base cycle (`slow` 60s, `normal` 36s, `fast` 18s). Even `high` and `fast` stay within a texture rather than a picture.
...
| `intensity` | `"low" \| "medium" \| "high"` | "medium" | Peak opacity of the ink: 12%, 20% or 32%. Lines cover little area, so even `high` reads as a texture. |
