...
[Card](/docs/components/card) is for content; `Panel` is the application chrome around a tool. A `PanelDescription` sits on its own line below the title and the actions.

## Variants

`default` has Card's hairline border, `elevated` a shadow, `flat` no edge and `outline` a visible border on a transparent background. `size` (`sm`, `md`, `lg`) scales the padding.
...
`PanelActions` holds one to three icon buttons. For more, put an [Overflow](/docs/tecton/overflow) `Toolbar` in it: the title then takes up to 60 % of the header and the actions move into a More menu as the panel narrows.
...
Styled `header`, `h2`, `p`, `div`, `div` and `footer` elements that accept the props of the element they render.
