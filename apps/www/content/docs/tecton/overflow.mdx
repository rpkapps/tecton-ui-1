...
<Callout>
  `Toolbar` has toolbar semantics: one tab stop, arrow keys move between the visible controls. Use `Overflow` when the row is not a toolbar semantically (a breadcrumb, a chip row). Both need their items and dividers as direct children.
</Callout>
...
Each item declares what it becomes in the menu with `overflow`. A button needs nothing: its menu item is built from `label`, `icon`, `shortcut` and `onAction`, and `onAction` is injected into the `Button` child, so the handler is passed once. Other controls pass their own menu form:
...
`Toolbar` renders with toolbar semantics (give it an `aria-label`); `Overflow` is a `div`. Both accept:
...
| `onAction` | `() => void` | - | Handler of the default menu item; also injected into a `Button` child. |
