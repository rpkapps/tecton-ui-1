...
Source: /docs/components/message.md
...
[`Bubble`](/docs/components/bubble.md). For the scroll container around a
conversation, use [`MessageScroller`](/docs/components/message-scroller.md).
...
For in-progress messages, use a [`Marker`](/docs/components/marker.md) with `role="status"` so assistive tech announces the update as it appears.
