...
<Callout variant="info">
  Components carry their own text sizes (for example `text-sm` on buttons and inputs), which coincide with Tecton's `actionMedium` / `medium` styles. Setting a size on a component by hand overrides the one its variant owns — use the `size` variant instead.
</Callout>
