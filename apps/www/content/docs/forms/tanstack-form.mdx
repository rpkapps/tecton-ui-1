---
title: TanStack Form
description: Build forms in React using TanStack Form, Zod and the React Aria components.
links:
...
It follows the [upstream shadcn/ui guide](https://ui.shadcn.com/docs/forms/tanstack-form); the examples are rewritten for the React Aria components, whose controls use `isSelected` / `onChange`, a Select's `value` / `onChange` and `isInvalid` instead of the Radix `checked` / `onCheckedChange`, `value` / `onValueChange` and `aria-invalid` props.
...
```tsx lineNumbers title="form.tsx"
...
```tsx lineNumbers title="form.tsx" {10-21}
...
<ComponentSource name="form-tanstack-demo" title="form.tsx" />
...
```tsx lineNumbers title="form.tsx" {13-15}
import { useForm } from "@tanstack/react-form"
...
```tsx lineNumbers title="form.tsx" {6-9}
const form = useForm({
  defaultValues: {
    title: "",
    description: "",
...
```tsx lineNumbers title="form.tsx" {4,18}
<form.Field
  name="email"
  children={(field) => {
...
```tsx lineNumbers title="form.tsx" {6,11-14,22}
<form.Field
  name="username"
  children={(field) => {
    const isInvalid = field.state.meta.isTouched && !field.state.meta.isValid
...
```tsx lineNumbers title="form.tsx" {6,13-16,24}
<form.Field
  name="about"
  children={(field) => {
    const isInvalid = field.state.meta.isTouched && !field.state.meta.isValid
...
- For select components, use `value` and `onChange` on the `<Select />` component, and label it with `aria-labelledby` pointing at the `FieldLabel`'s `id` (the trigger's own labelling overrides `htmlFor`). React Aria selects use `null` for "nothing selected", so map the empty string to `null` and back.
...
```tsx lineNumbers title="form.tsx" {6,18-22}
<form.Field
  name="language"
  children={(field) => {
    const isInvalid = field.state.meta.isTouched && !field.state.meta.isValid
...
```tsx lineNumbers title="form.tsx" {12,17,22-23,44}
<form.Field
  name="tasks"
  mode="array"
  children={(field) => {
...
```tsx lineNumbers title="form.tsx" {14-15,21}
<form.Field
  name="plan"
  children={(field) => {
    const isInvalid = field.state.meta.isTouched && !field.state.meta.isValid
...
```tsx lineNumbers title="form.tsx" {6,14,19-20}
<form.Field
  name="twoFactor"
  children={(field) => {
    const isInvalid = field.state.meta.isTouched && !field.state.meta.isValid
...
```tsx lineNumbers title="form.tsx" {3,12-14}
<form.Field
  name="emails"
  mode="array"
  children={(field) => {
