...
Use the `variant` prop: `default` with [Card](/docs/components/card)'s hairline ring, `elevated` with a shadow, `flat` without an edge and `outline` with a visible ring on a transparent background.
