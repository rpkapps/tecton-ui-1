...
Style states with `data-selected` and `data-disabled` on a chip and `data-empty` on the list. They are presence attributes; write the selected state as `data-[selected]:…`, since shadcn's `data-selected:` variant matches only `"true"`.
