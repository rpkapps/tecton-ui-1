...
`AppShellAction` is a ghost icon button whose `label` is both the accessible name and the tooltip; `shortcut` adds a key cap to the tooltip. `AppShellCommandTrigger` looks like a search field from `md` up and collapses to an icon button below it; `shortcut` shows a key hint from `lg` up. Both hints only draw the key: binding it is up to the application. `AppShellOverflow` is the "More" menu for the actions a narrow header cannot show, and `AppShellDivider` is the hairline between groups.
...
Ghost icon `Button` with a tooltip below it that also opens on keyboard focus. Accepts `Button` props (`onClick`, `disabled`, …).

| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `label` | `string` | - | Accessible name and tooltip text. |
| `shortcut` | `ReactNode` | - | Key hint in the tooltip: a string is one key cap (`"?"`), a node is drawn as is (`<KbdGroup>…</KbdGroup>`). |
...
| `shortcut` | `ReactNode` | - | Key hint at the end, from `lg` up: a string is one key cap (`"⌘K"`), a node is drawn as is. It only displays the key; bind it in the application. |
...
| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `open` / `defaultOpen` / `onOpenChange` | `boolean` / `boolean` / `(open: boolean) => void` | - | Open state of the menu. |
| `className` | `string` | - | Classes of the trigger button. |
