...
<Callout>
  **One inversion level.** A light island inside a dark page works, and so does a dark island inside a light page. A dark island inside a light island inside a dark page does not — the variant reads it as light. Two inversions are not supported.
</Callout>

What an inverted section switches, and what it does not:

| Layer | Follows `.light` / `.dark` on a subtree |
| --- | --- |
| `color-scheme`, the raw `--tecton-*` tokens and the palette ramps | Yes — `tecton-tokens.css` and `tecton-palette.css` declare both modes on `:root, [data-theme="light"], .light` and `.dark, [data-theme="dark"]`, so palette utilities (`bg-blue-560`, `text-gray-830`) switch with the section. |
| The `dark:` utility variant | Yes, with the variant above. |
| The shadcn variables (`--background`, `--primary`, …) | Only for `.dark`. `globals.css` declares them in `:root` and `.dark` only, and a custom property is substituted on the element that declares it — so a `.light` subtree of a dark page keeps the dark values it inherits. Set the tokens the section needs on it (`class="light [--background:var(--tecton-palette-gray-50)]"`), or, for a mounted application, import [`scoped-theme.css`](/docs/micro-frontends#theme-variables), which declares the full set on `[data-tecton-root]` for both modes. |
