...
# Avatar — @tecton/react/components/avatar
...
- a glyph that stands for a thing rather than a person → `Tecton icons` (tecton docs tecton-icons)
- a colour value shown as a square → `ColorSwatch` (tecton docs color-swatch)
- a number pinned to the corner of an icon → `CountBadge` (tecton docs count-badge)
- a person tag the user can select or remove → `Chip` (tecton docs chip)
