...
// `cva()`, never into DayPicker's `classNames` map, so the entries that carry
// a style class go through a cva.
const calendarDropdownRootVariants = cva(
  "relative rounded-(--cell-radius) border border-input has-focus:border-ring has-focus:ring-2 has-focus:ring-ring"
)

const calendarCaptionLabelVariants = cva("font-medium select-none", {
  variants: {
    captionLayout: {
      label: "text-sm",
      dropdown:
        "flex h-8 items-center gap-1 rounded-(--cell-radius) pe-1 ps-2 text-sm [&>svg]:size-3.5 [&>svg]:text-muted-foreground",
    },
  },
})
      ...
      classNames={{
        ...
        caption_label: cn(
          calendarCaptionLabelVariants({
            captionLayout: captionLayout === "label" ? "label" : "dropdown",
          }),
