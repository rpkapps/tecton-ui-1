...
Navigation — where the user is and where they can go. Start from the table, then read the section of the component it sends you to: Breadcrumb, Sidebar, Pagination, AppFinder. Each one is imported from its own module; `@tecton/react` has no root export.
...
- the global action cluster at the end of the shell header: use `AppShellActions` — `import { AppShellActions } from "@tecton/react/tecton/app-shell"`
