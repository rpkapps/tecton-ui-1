...
Structural parts; `PanelActions` sits at the end of the header, `PanelContent` scrolls. The header row wraps: `PanelDescription` takes a line of its own below the title and the actions, so the title keeps the width the actions leave.
