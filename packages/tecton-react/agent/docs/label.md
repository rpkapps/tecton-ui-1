...
# Label — @tecton/react/components/label
...
- the label of a control that has a description or an error → `FieldLabel` (tecton docs field)
- the caption over a group of related controls → `FieldLegend` (tecton docs field)
- a heading beside a control that is already inside a label → `FieldTitle` (tecton docs field)
