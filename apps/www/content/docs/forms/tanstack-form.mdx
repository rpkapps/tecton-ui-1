...
It follows the [upstream shadcn/ui guide](https://ui.shadcn.com/docs/forms/tanstack-form), with the components imported from `@tecton/react`.
...
- For select components, use `field.state.value` and `field.handleChange` on the `<Select />` component, and pass the options as `items` so the trigger shows the selected label. `null` is "nothing selected": map the empty string to `null` and back.
...
```tsx lineNumbers title="form.tsx" {6,18-20,24}
<form.Field
  name="language"
  children={(field) => {
    const isInvalid = field.state.meta.isTouched && !field.state.meta.isValid
...
```tsx lineNumbers title="form.tsx" {12,17,22-24,44}
<form.Field
  name="tasks"
  mode="array"
  children={(field) => {
...
```tsx lineNumbers title="form.tsx" {21,29,35}
<form.Field
  name="plan"
  children={(field) => {
    const isInvalid = field.state.meta.isTouched && !field.state.meta.isValid
...
```tsx lineNumbers title="form.tsx" {6,14,19-21}
<form.Field
  name="twoFactor"
  children={(field) => {
    const isInvalid = field.state.meta.isTouched && !field.state.meta.isValid
