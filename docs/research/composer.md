...
| 6 | A **global "focus composer" shortcut** must use a modifier (Shift+Esc like ChatGPT, or `mod+/`), be bound by the application, and be listed in its help. No single-character shortcut like "/" unless it can be turned off or only works when nothing editable is focused. | 2.1.4. Tecton binds no keys, so the binding is the application's. |
...
- Global focus shortcut: the application's own binding (Tecton binds no keys), calling `focus` from `useComposer`.
