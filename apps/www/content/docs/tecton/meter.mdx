...
Or use a fixed `color`. `default` fills with the shared `progress` token, the same as the linear [Progress](/docs/components/progress), and the unfilled segments are that token at 38 %.
...
| `color` | `"default" \| "success" \| "warning" \| "error" \| "info" \| "auto" \| "custom"` | "default" | Fill colour; `default` is the `progress` token. |
