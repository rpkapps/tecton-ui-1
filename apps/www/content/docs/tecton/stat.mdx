...
The value is set in the mono font with tabular numbers, the unit in the sans font.
...
`trend` picks the icon, `tone` the colour: `tone` says whether the change is good news. It defaults to the trend's (up `positive`, down `negative`), so set it where down is good — cost, CAPEX, downtime, risk.

<ComponentPreview name="stat-delta" />

## Group

`StatGroup` lays several stats out in a responsive grid.
...
A `div`; accepts its props.

| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `size` | `"sm" \| "md" \| "lg"` | `"md"` | Value size 14 / 20 / 28 px. |
...
| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `unit` | `ReactNode` | - | Unit after the value. |

### StatDelta

Trend icon and text; carries `data-trend` and `data-tone`.

| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `trend` | `"up" \| "down" \| "flat"` | `"flat"` | Which way the number moved. |
| `tone` | `"positive" \| "negative" \| "neutral"` | from `trend` | Whether the move is good or bad. |

### StatLabel, StatHelp, StatGroup

Styled `span`, `span` and `div` elements.
