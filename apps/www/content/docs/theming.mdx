...
Toggle the mode with the **`dark` class** on `<html>`. This site uses `next-themes` with `attribute="class"`. `data-theme="dark"` is not an alternative: in `globals.css` it only switches the `--tecton-*` layer (the raw token export and the palette), while the shadcn variables are declared in `:root` and `.dark` and the `dark:` utility variant resolves to `&:is(.dark *)`, so both keep reading light values. The one place the attribute is equivalent is `@tecton/react/styles/scoped.css`, which honours the class and the attribute on the root element of a mounted application (see [Micro-frontends](/docs/micro-frontends)).
