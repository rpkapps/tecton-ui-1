...
`pnpm --filter @tecton/react tokens:build` writes `tecton-palette.css` (the ramps, and the Tailwind `@theme` that exposes them) and `tecton-theme.css`, patches the CLI-managed `globals.css` in place (the variable values in `:root`, `.dark` and `@theme inline`, the `@import`s, the `dark` custom variant and the `.light` block that mirrors `:root` — nothing else) and the mapping document.
...
Toggle the mode with the **`dark` class** on `<html>`. This site uses `next-themes` with `attribute="class"`. `data-theme="dark"` on the document root works too — the raw token export and the palette both key on `.dark, [data-theme="dark"]`, the shadcn variables in `:root` re-read them there, and the `dark:` utility variant (below) matches the attribute as well. Use the class: it is what the shadcn ecosystem, `next-themes` and every example assume, and on a **subtree** the attribute is not equivalent — see [Inverted sections](#inverted-sections).

### Inverted sections

A section can run in the opposite mode to the page: `<div class="light">` inside a dark page, `<div class="dark">` inside a light one, or [`ThemeRoot`](/docs/tecton/theme-root) with `theme="light"` / `theme="dark"` for a mounted application. The `dark:` utility variant follows the **nearest** theme marker rather than the outermost one:

```css
/* globals.css and scoped.css, both written by tokens:build */
@custom-variant dark (&:where(.dark *, [data-theme="dark"] *):not(:where(:is(.dark, [data-theme="dark"]) :is(.light, [data-theme="light"]), :is(.dark, [data-theme="dark"]) :is(.light, [data-theme="light"]) *)));
```

An element matches when it is inside a dark marker and not inside a light marker that sits within it. The `:where()` keeps the whole selector at the specificity of the utility class alone, `:not()` included, so nothing changes weight relative to an application's own overrides. Writing `.light` on a page root is fine too (it is what `next-themes` emits for the light theme): a dark island inside it still matches.

The line is the CLI's to create and `tokens:build`'s to keep correct — the shadcn CLI writes `&:is(.dark *)` once, from a string hard-coded in its own transform, and never touches a file that already has a `@custom-variant`. `tokens:check` fails if `globals.css` and `scoped.css` ever stop agreeing on it.

### The light block

The variant is only half of it. `globals.css` also carries a `.light, [data-theme="light"]` block that repeats every `:root` declaration, immediately after `.dark`:

```css
:root { --primary: var(--tecton-color-action-primary-bg); /* … */ }
.dark { --primary: var(--tecton-color-action-primary-bg); /* … */ }
.light, [data-theme="light"] { --primary: var(--tecton-color-action-primary-bg); /* … */ }
```

Three identical-looking blocks, and all three are needed. A custom property's computed value is its specified value **with `var()` already substituted**, resolved on the element that declares it; a descendant inherits the result, not the reference. That is why `.dark` repeats `:root` instead of leaving it to inheritance — the repetition is what makes the dark element re-read the dark tokens — and it is why light needs the same treatment: without the third block a `.light` element inside a dark page would re-declare the raw `--tecton-*` tokens and still inherit `--primary` frozen at the dark value.

Both `.dark` and `.light, [data-theme="light"]` weigh a single class or attribute, so source order decides for an element that carries both markers; light comes last and wins, matching how `scoped-theme.css` orders its own blocks. `tokens:check` fails if the light block stops mirroring `:root` declaration for declaration, or moves ahead of `.dark`.

The three blocks are generated, so this is not something to hand-write: `tokens:build` rebuilds the light block from `:root` on every run, in `globals.css`, in `tecton-theme.css` and in `registry/theme.json` (where it rides in the item's `css`, since `cssVars` can only reach `:root` and `.dark`).

### What switches

Everything an inverted section renders with:

| Layer | Follows `.light` / `.dark` on a subtree |
| --- | --- |
| `color-scheme`, the raw `--tecton-*` tokens and the palette ramps | Yes — `tecton-tokens.css` and `tecton-palette.css` declare both modes on `:root, [data-theme="light"], .light` and `.dark, [data-theme="dark"]`, so palette utilities (`bg-blue-560`, `text-gray-830`) switch with the section, and `light-dark()` follows it too. |
| The shadcn variables (`--background`, `--primary`, …) | Yes, through the light block above — at any nesting depth, since every marker re-declares the whole set on itself. |
| The `dark:` utility variant | Yes, for the first inversion. |

<Callout>
  **The variant handles one inversion.** The variables invert as deep as you nest them; the `dark:` variant reads one level. A *dark* section inside a light section inside a dark page — the third marker — keeps its dark variables but has its `dark:` utilities switched off, so anything whose dark styling comes from a `dark:` class rather than from a token renders light there. One inversion is the supported depth.
</Callout>
