...
| `g w` | A sequence: `g` then `w` within a second, shown as `G then W` (read as "G, then W"). |
| `escape`, `enter`, `arrowup` | Named keys (`esc`, `up`, `space` are aliases). |

Letters and digits match the key pressed when the character typed hides it: `alt+k` still fires where Option turns K into "˚" on a Mac, and `g w` works on a non-Latin layout. Other Latin layouts match by the character, so a letter stays where the layout puts it.

Shortcuts without Ctrl, ⌘ or Alt do not fire while an input, textarea or editable element has focus (inside a shadow root too), so `n` never steals a letter from a form. Set `allowInInput` to change that for one shortcut.

A held key fires its shortcut once: the key's auto-repeats are swallowed (so the browser's own action does not run either) but do not call `onAction` again, so a toggle does not flicker. Set `allowRepeat` for a shortcut that should keep firing while the key is held, such as a nudge or a zoom step.

## Listing shortcuts

`useShortcuts()` returns the registered shortcuts and re-renders when they change; the shell's command palette and `?` dialog are built on it. `ShortcutKeys` renders a shortcut as key caps: the keys of a chord joined with "+" (`Ctrl + K`, ⌘ on Apple keyboards), the steps of a sequence with "then" (`G then W`). Screen readers get the same as text and the caps are hidden from them, so no key is read out twice.
...
A nested provider normally reuses its parent's registry, so a component can wrap itself in one and still share the host's shortcuts. A nested provider given a `target` but no `registry` gets a registry of its own, listening on that target only: the parent's registry already listens on its own target, and one registry listening in two places would see every key twice. The scoped shortcuts are then listed by `useShortcuts()` inside that provider, not in the host's lists.
...
- the header button whose tooltip shows a shortcut: use [AppShellAction](/docs/tecton/app-shell)
...
A nested provider without a `registry` reuses the parent's, unless it has a `target` of its own: then it creates its own registry and listens on that target.
...
| `allowRepeat` | `boolean` | false | Fire again while the key is held (auto-repeat). Without it a held key fires once; the repeats are still swallowed. |
...
### useShortcuts, formatShortcut, ShortcutKeys, useIsMacPlatform

`useShortcuts()` returns the live `Shortcut[]`. `formatShortcut(keys)` returns the key caps per chord (`[["⌘", "K"]]`) for custom rendering; `ShortcutKeys` renders them with `Kbd`. `useIsMacPlatform()` says whether the reader's keyboard is an Apple one, for a hand-written hint that shows ⌘ instead of Ctrl; it is `false` on the server and in the first render, so hydration matches.
