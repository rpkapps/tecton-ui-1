...
**Do not use `<Button>` for links.** `Button` always renders with `role="button"`, which overrides the semantic link role on `<a>` elements. Use `buttonVariants` with a plain `<a>` tag instead.
