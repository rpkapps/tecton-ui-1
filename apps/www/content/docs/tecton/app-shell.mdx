...
`AppShellActions` is the global action cluster at the end of the header: the command palette trigger, the icon actions (help, settings, release notes, bug report) and the user menu. It is `ml-auto`, so it pins itself to the end of the header.
...
`AppShellAction` is a ghost icon button whose `label` is both the accessible name and the tooltip; `shortcut` adds the key hint in [Shortcuts](/docs/tecton/shortcuts) key syntax. `AppShellCommandTrigger` looks like a search field from `md` up and collapses to an icon button below it. `AppShellOverflow` is the "More" menu for the actions a narrow header cannot show, and `AppShellDivider` is the hairline between groups.
