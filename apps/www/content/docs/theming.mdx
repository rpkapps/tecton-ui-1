...
Toggle the mode with the **`dark` class** on `<html>`; this site uses `next-themes` with `attribute="class"`. `data-theme="dark"` on the document root works the same way.

### Inverted sections

A section can run in the opposite mode to the page: put the `light` or `dark` class on any element — or `theme="light"` / `theme="dark"` on [`ThemeRoot`](/docs/tecton/theme-root) for a mounted application — and everything below it follows: `color-scheme`, the raw `--tecton-*` tokens, the palette ramps, the shadcn variables and the `dark:` utilities. Inside a page, use the class: `data-theme="dark"` switches a whole document, not a subtree.
...
<Callout>
  **One inversion for the `dark:` utilities.** A light island in a dark page renders completely, and so does a dark island in a light one. Nest a third marker inside that and its variables still follow it, while its `dark:` utilities read the inversion above.
</Callout>
