...
upstream: apps/v4/content/docs/components/base/button.mdx
---
...
**Do not use `<Button render={<a />} nativeButton={false} />` for links.** `Button` always applies `role="button"`, which overrides the semantic link role on `<a>` elements. Use `buttonVariants` with a plain `<a>` tag instead.
...
<Dont severity="HIGH" title="onPress and isDisabled on a Button">
